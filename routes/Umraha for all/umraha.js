...
module.exports.add = async (req, res) => {
    try {
        const images = req.files && req.files['images']
            ? req.files['images'].map(file => `${BASE_URL}/images/${file.filename.replace(/\\/g, '/')}`) // Use backticks for template literals
            : (req.body.images && typeof req.body.images === 'string' ? JSON.parse(req.body.images) : []);

        console.log(req.files, "farhan");

        const thumbnail = req.files && req.files['thumbnail']
            ? `${BASE_URL}/thumbnails/${req.files['thumbnail'][0].filename.replace(/\\/g, '/')}` // Use backticks for template literals
            : (req.body.thumbnail || '');
